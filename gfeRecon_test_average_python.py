...
for rx_card in range(chassis_cards):
    for num_channels in range(0, num_channels_per_card):
        sig[num_channels + rx_card * num_channels_per_card, :] = bdata[rx_card, np.arange(0 + num_channels * 2, bdata[rx_card, :].size, sample_skip)]
        sig_complex[num_channels + rx_card * num_channels_per_card, :] = (bdata[rx_card, np.arange(1 + num_channels * 2, bdata[rx_card, :].size, sample_skip)])
...
if UI['signalAverages'] > 1:
    ...
    avg_sig_real[:, :, :, :, 0] = sig
    avg_sig_complex = np.zeros((sig_complex1.shape[0], sig_complex1.shape[1], sig_complex1.shape[2], number_of_slices, UI['signalAverages']), float)
...
f_filter = fermi.fermi(np.shape(sig)[0],round(0.8 * float(np.shape(sig)[0]),0),round(0.1* float(np.shape(sig)[0]),0),0)
